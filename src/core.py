...
def generate_stitch_settings(manifest: DatasetManifest, output_dir: str, data_path: str, use_tiles_view: bool):
    """
    ...
    step_x = int(manifest.width_px * (1 - overlap_x_frac))
    step_y = int(manifest.height_px * (1 - overlap_y_frac))
    ...
    lines.append("output_format = raw")
    lines.append("")
    ...
    for t in range(n_tiles):
        ...
        lines.append(f"{filename} = {pos_x}, {pos_y}, {pos_z}")
...
def generate_local_script(manifest: DatasetManifest, output_dir: str, conda_config: Dict[str, str], embed_pi2_path: Optional[str] = None):
    """
    Generates a local execution script (run_local.bat for Windows and run_local.sh for POSIX).
